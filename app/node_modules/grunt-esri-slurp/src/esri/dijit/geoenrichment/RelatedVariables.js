...
define(["../../declare", "./BaseSelectComparison", "./dom", "dojo/dom-construct", "dojo/dom-attr", "./lang", "dojo/i18n!../../nls/jsapi", "dojo/dom-class", "dojo/query", "dojo/string", "dojo/number", "./formatVariable"], function(u, v, n, s, h, w, g, l, y, t, x, k) {
    ...
    return u("esri.dijit.geoenrichment.RelatedVariables", [v], {
        ...
        updateUIExpanded: function() {
            ...
            var b = this._calculate(),
                c = Math.max(Math.abs(b.minDif), Math.abs(b.maxDif)),
                a, e;
            for (e = this.table.rows.length; e < b.indexes.length + 1; e++) {
            ...
            var f;
            for (e = 0; e < b.rows.length; e++) f = this.getFieldByIndex(b.indexes[b.rows[e][0]]), a = this.table.rows[e + 1], a.cells[0].innerHTML = b.rows[e][1], a.cells[1].innerHTML = k(f, b.rows[e][2]), d = b.rows[e][3], w.isNumber(d) ? (f = 0 < d ? "+" + k(f, d) : 0 > d ? "-" + k(f, -d) : "0", a.cells[2].innerHTML = f, a.cells[2].className = "RelatedVariables_DifferenceColumn", 0 < d ?
        ...
        updateUICollapsed: function() {
            ...
            var b = this._calculate(),
                c, a;
            for (a = this.table.rows.length; a < b.indexes.length + 1; a++) c = this.table.insertRow(-1), 0 < a && 0 === a % 2 && (c.className = "AlternatingRow"), h.set(c.insertCell(-1), "class", "RelatedVariables_TextColumn"),
