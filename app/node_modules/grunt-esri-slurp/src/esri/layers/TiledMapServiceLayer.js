//>>built
define(["dojo/_base/declare", "dojo/_base/connect", "dojo/_base/lang", "dojo/_base/array", "dojo/_base/url", "dojo/dom-construct", "dojo/dom-class", "dojo/dom-geometry", "dojo/dom-style", "dojox/collections/ArrayList", "dojox/gfx/matrix", "../kernel", "../config", "../sniff", "../domUtils", "../tileUtils", "../geometry/Point", "../geometry/Rect", "../geometry/Extent", "./layer"], function(P, v, E, A, J, r, K, L, l, M, N, q, Q, x, O, F, z, B, R, S) {
    var C = Q.defaults.map.zoomDuration;
    return P(S, {
        declaredClass: "esri.layers.TiledMapServiceLayer",
        constructor: function(a,
            c) {
            v.connect(this, "onLoad", this, "_initTiledLayer");
        ...
        _setMap: function(a, c, d, b) {
            ...
            "css-transforms" === a.navigationMode ? (t[g.transform] = q._css.translate(-f.x, -f.y), l.set(e, t), delete t[g.transform], t[g.transition] = g.transformName + " " + C + "ms ease", l.set(this._active = r.create("div", null, e), t), this._active._remove = 0, this._passives = []) : (t.left = -f.x + "px", t.top = -f.y + "px", l.set(e, t));
            ...
            if (this.suspended && !a.loaded) var m = v.connect(a, "onLoad", this, function() {
                v.disconnect(m);
                m = null;
                this.setExclusionAreas(this.exclusionAreas);
                this.evaluateSuspension()
        ...
        _onExtentChangeHandler: function(a, c, d, b) {
            ...
            if (!c._isPanningOrZooming()) {
                if ("css-transforms" === c.navigationMode) {
                    if (d)
                        for (b = this._passives.length - 1; 0 <= b; b--) f = this._passives[b], l.set(f, q._css.names.transition, "none"), f._marked ? (this._passives.splice(b, 1), f.parentNode && f.parentNode.removeChild(f),
                ...
                b = a.x;
                d = a.y;
                "css-transforms" === c.navigationMode ? (e = {}, e[q._css.names.transform] = q._css.translate(b, d), l.set(this._div, e)) : l.set(this._div, {
                    left: b + "px",
                    top: d + "px"
                });
                this.__coords_dx = b;
        ...
        _onPanHandler: function(a, c) {
            ...
            "css-transforms" === d.navigationMode ? (d = {}, d[q._css.names.transform] = q._css.translate(b.x, b.y), l.set(this._div, d), !x("esri-touch") && !x("esri-pointer") && this._updateImages({
                x: -b.x,
                y: -b.y,
                width: b.width,
                height: b.height
        ...
        _onScaleHandler: function(a, c) {
            ...
            for (d = this._passives.length - 1; 0 <= d; d--) {
                ...
                0 === h.childNodes.length ? (this._passives.splice(d, 1), r.destroy(h)) : ("none" === h.style[e.transition] && l.set(h, e.transition, e.transformName + " " + C + "ms ease"), l.set(h, e.transition, c ? "none" : e.transformName + " " + C + "ms ease"), h._matrix =
                    a, b[e.transform] = q._css.matrix(h._multiply ? N.multiply(a, h._multiply) : a), l.set(h, b))
            }
            this._active && 0 === this._active.childNodes.length || (l.set(this._active, e.transition, c ? "none" : e.transformName + " " + C + "ms ease"), this._active._matrix = a, b[e.transform] = q._css.matrix(this._active._matrix), l.set(this._active, b), this._passives.push(this._active), b = {
                position: "absolute",
                width: f.width + "px",
                height: f.height + "px",
                overflow: "visible"
        ...
        _updateImages: function(a) {
            if (this._ct) {
                ...
                for (w = n; w <= G; w++)
                    for (n = q; n <= a; n++) y = g + n, u = h + w, this._wrap && (u < I ? (u %= H, u = u < I ? u + H : u) : u > D && (u %= H)), !this._isExcluded(l, y, u) && (y >= x && y <= z && u >= C && u <= E) && (c = r + "_" + v + "_tile_" + l + "_" + n + "_" + w, -1 === F(k, c) && (p.add(c), k.push(c), s(l, n, y, w, u, c, d, b, m, e, f)))
            }
        },
        _cleanUpRemovedImages: function() {
            ...
            if ("css-transforms" === this._map.navigationMode)
                for (d = this._passives.length - 1; 0 <= d; d--) {
                    ...
                    0 === e.childNodes.length ? (this._passives.splice(d, 1), c(e)) : this._map.fadeOnZoom && (!e._marked && e._remove === e.childNodes.length) && (l.set(e, b.transition, "opacity 0.65s"), l.set(e, "opacity", 0), e._marked = 1, v.disconnect(e._endHandle), e._endHandle = v.connect(e, b.endEvent, this._transitionEnd))
                }
            a.clear()
        },
        _transitionEnd: function(a) {
            var c =
                a.target;
            "opacity" === a.propertyName && (v.disconnect(c._endHandle), c._endHandle = null, a = A.indexOf(this._passives, c), -1 < a && this._passives.splice(a, 1), c.parentNode && c.parentNode.removeChild(c), r.destroy(c))
        },
        _addImage: function(a, c, d, b, e, f, h, g, t, m, k) {
            if (this._patchIE) m = this._tiles[f] = r.create("div"), m.id = f, K.add(m, "layerTile"), l.set(m, {
                ...
                "css-transforms" === c.navigationMode ? (h[s.transform] = q._css.translate(b, k), l.set(m, h), m._left = b, m._top = k) : (h.left = b + "px", h.top = k + "px",
                    l.set(m, h));
                ...
                else if (f = this.getTileUrl(a, d, e, m)) this._failedRequests && this._failedRequests[f] ? (l.set(m, this._failedRequests[f].css), m.src = this._failedRequests[f].src, this._multiple = parseInt(this._failedRequests[f].css.width) /
                    this._tileW) : (this._multiple = 1, m.src = f);
        ...
        addTimestampToURL: function(a) {
            ...
            c && (a = this._reCheckTS.test(a) ?
                a.replace(this._reReplaceTS, "$$$1" + c) : a + ((-1 === a.indexOf("?") ? "?" : "\x26") + "_ts\x3d" + c));
            return a
        },
        refresh: function() {
        ...
        _tileErrorHandler: function(a, c, d) {
            ...
            else if (b = new J(d.src), b = b.path.split("/"), b = parseInt(b[b.length - 3]), e = this._ct.lod.level - b + 1, this._multiple = Math.pow(2, e), b === this._lowestLevel ||
                0 === this._resamplingTolerance || this._resamplingTolerance && Math.log(this._multiple) / Math.LN2 > this._resamplingTolerance) f = !1;
                    ...
                    for (m = a.length; l < m; l++)
                        if (g = a[l], (n = g.geometry) && "extent" === n.type && n.xmin <= n.xmax && n.ymin <= n.ymax) {
                            if (!c.equals(n.spatialReference))
                                if (c._canProject(n.spatialReference)) c.isWebMercator() ? (q = z.lngLatToXY(n.xmin, n.ymin), n = z.lngLatToXY(n.xmax, n.ymax)) : (q = z.xyToLngLat(n.xmin, n.ymin, !0), n = z.xyToLngLat(n.xmax,
                                    n.ymax, !0)), n = new R(q[0], q[1], n[0], n[1], c);
                                else continue;
                            r = -1;
