...
define(["dojo/_base/declare", "dojo/_base/connect", "dojo/_base/array", "dojo/_base/Color", "dojo/_base/lang", "dojo/has", "dojo/on", "../kernel", "../request", "../graphic", "./FeatureLayer", "./StreamMode", "./StreamTrackManager", "../geometry/jsonUtils", "../symbols/SimpleFillSymbol", "../symbols/SimpleLineSymbol", "../symbols/SimpleMarkerSymbol", "../renderers/SimpleRenderer", "./PurgeOptions"], function(u, v, g, l, p, C, w, D, x, y, q, z, A, m, r, h, s, B, t) {
    return u([q], {
        ...
        _initLayer: function(a, c) {
            ...
            if (a) {
                ...
                if (a.layerDefinition) this.purgeOptions =
                    new t(this, this._params.purgeOptions || {}), this.socketUrl = this._params.socketUrl || a.layerDefinition.socketUrl || void 0;
                else {
                    if (this._params.socketUrl) this.socketUrl = this._params.socketUrl;
                    else {
                        var d = this._getWebsocketConnectionInfo(a),
                            e = d.urls;
                        e && e.length ? (this._socketUrls = e, this.socketUrl = e[0], this.socketDirection = "broadcast" === this._params.socketDirection ? "broadcast" : "subscribe", this.socketUrl += "/" + this.socketDirection, this._websocketToken = d.token, e.length > this.maxReconnectAttempts && (this.maxReconnectAttempts =
                            e.length)) : (this.socketUrl = void 0, d = "No web socket urls were retrieved from the Stream Service. Layer will not attempt to connect.", "https:" === location.protocol && (d += " An insecure web socket connection cannot be made from a secure web page."), this.onError(Error(d)))
                    }
                    ...
                    if (this._params.geometryDefinition ||
                        this._outFields || this._defnExpr) {
                        b = b || {};
                        ...
                        try {
                            ...
                            this.onError(Error("Error trying to create filter object: " + f + ". Layer will not have filter applied")), this._filter = null
                        }
                    }
                }
                ...
                (this._params.refreshRate || 0 === this._params.refreshRate) && this._mode &&
                    this._mode._setRefreshRate && this._mode._setRefreshRate(this._params.refreshRate);
        ...
        _unsetMap: function(a, c) {
            g.forEach(this._connects, v.disconnect);
        ...
        clear: function() {
            try {
                this._mode && this._mode._clearDrawBuffer && this._mode._clearDrawBuffer(),
        ...
        connect: function(a) {
            ...
            try {
                if (!this._connected) {
                    ...
                    if (this._keepLatestUrl && !this._keepLatestQueried && this._mode._fetchArchive) return b = this.on("update-end", function() {
                    ...
                    if (e)
                        for (f in e) null !== e[f] && (k = "geometry" === f ? JSON.stringify(e[f]) : e[f], d.push(f + "\x3d" + k));
                    0 < d.length && (g += "?" + d.join("\x26"));
                    return n = this._createConnection(g, a)
                }
            } catch (h) {
        ...
        getFilter: function() {
            ...
            a && g.forEach(b, function(b) {
                var e = a[b];
                e && ("geometry" === b ? e = m.fromJson(e) : "outFields" === b && (e = e.split(",")), c[b] = e)
            });
            return c
        },
        ...
        _createConnection: function(a, c) {
            ...
            d.onclose = function(a) {
                var c, d = !0,
                    g = b._connected,
                    n = null;
                if (b._connected || b._reconnecting || !b.socket) {
                    if (a.code)
                        if (c = "Connection failed: ", 4400 === a.code) c +=
                            a.reason || "Invalid url parameters. Check filter properties.", d = !1;
                        else if (4404 === a.code) c += "Service not found", d = !1;
                    else if (4401 === a.code || 4403 === a.code) c += "Not authorized", d = !1;
                    d && (b._reconnectAttempts += 1, b._reconnectAttempts > b.maxReconnectAttempts && (c = "Maximum reconnect attempts exceeded", d = !1));
                    b._connected = !1;
                    g && (c && (n = Error(c)), b.onDisconnect({
                        error: n,
                        willReconnect: d
                    }));
                    d ? b._attemptReconnect() : b.socket = null
                } else b.socket = null, b._connected = !1
            };
        ...
        _update: function(a) {
            ...
            b && (a.attributes && b.setAttributes(a.attributes), a.geometry && b.setGeometry(m.fromJson(a.geometry)))
        },
        _makeFilter: function(a) {
            ...
            if (void 0 !== a.geometry)
                if (b = b || {}, null === a.geometry) b.geometry = null;
                    ...
                    c = "string" ===
                        typeof a.geometry ? m.fromJson(JSON.parse(a.geometry)) : a.geometry.declaredClass ? a.geometry : m.fromJson(a.geometry);
        ...
        _makeOutFields: function(a) {
            ...
            a = c._getOutFields();
            g.forEach(a, function(a) {
                c._getField(a) && -1 === g.indexOf(b, a) && b.push(a)
            });
            e.fields = b;
            e.errors = d;
            return e
        ...
        _getWebsocketConnectionInfo: function(a) {
            var c = {
                ...
                e = [],
                k, f, h;
            a.streamUrls && g.forEach(a.streamUrls, function(a) {
            ...
            a = "https:" === location.protocol || 0 < e.length ? e : d;
            if (1 < a.length)
                for (k = 0; k < a.length - 1; k++) f = k + Math.floor(Math.random() * (a.length - k)), h = a[f], a[f] = a[k], a[k] = h;
            c.urls = a;
            return c
        },
        ...
        _setDefaultRenderer: function() {
            var a = this.geometryType,
                c = new l([5, 112, 176, 0.8]),
                b = new l([255, 255, 255]),
                b = new h(h.STYLE_SOLID, b, 1),
                d;
            if ("esriGeometryPoint" === a || "esriGeometryMultipoint" === a) d = new s(s.STYLE_CIRCLE, 10, b, c);
            else if ("esriGeometryPolyline" === a) d = new h(h.STYLE_SOLID,
                c, 2);
            else if ("esriGeometryPolygon" === a || "esriGeometryEnvelope" === a) c = new l([5, 112, 176, 0.2]), b = new l([5, 112, 176, 0.8]), b = new h(h.STYLE_SOLID, b, 1), d = new r(r.STYLE_SOLID, b, c);
        ...
        _makeObjectIdField: function() {
            ...
            if (!this.objectIdField) {
                ...
                for (; - 1 !== g.indexOf(d, "objectid_" + a);) a += 1;
                this.objectIdField = "objectid_" + a
            }
        },
        _featureHasOID: function(a, c) {
            return a.attributes && (a.attributes[c] || 0 ===
                a.attributes[c])
        }
    })
});