...
define(["dojo/_base/declare", "dojo/_base/lang", "dojo/_base/array", "dojo/_base/json", "dojo/_base/Deferred", "dojo/has", "../kernel", "../lang", "../layerUtils", "../deferredUtils", "./Task", "../geometry/Polygon", "../renderers/SimpleRenderer", "../geometry/scaleUtils", "./Geoprocessor", "./PrintTemplate", "dojo/dom-construct", "dojox/gfx/_base", "dojox/gfx/canvas", "dojox/json/query", "require", "require"], function(x, h, n, s, y, z, J, t, u, A, B, C, D, E, F, G, H, v, w, I) {
    return x(B, {
        ...
        execute: function(b, f, a) {
            ...
            if (g) {
                this.legendAll = !1;
                g.legendLayers ? n.forEach(g.legendLayers, function(a) {
                    var b = {};
                    b.id = a.layerId;
                ...
                var p, q;
                if ("Miles" === g.scalebarUnit || "Kilometers" === g.scalebarUnit) p = "esriKilometers", q = "esriMiles";
                else if ("Meters" === g.scalebarUnit || "Feet" ===
                    g.scalebarUnit) p = "esriMeters", q = "esriFeet";
                l = {
                    esriMiles: "mi",
                    esriKilometers: "km",
                    esriFeet: "ft",
            ...
            p = this._getPrintDefinition(b.map);
            b.outSpatialReference && (p.mapOptions.spatialReference = b.outSpatialReference.toJson());
            b.template && t.isDefined(b.template.showAttribution) &&
                (p.mapOptions.showAttribution = b.template.showAttribution);
            h.mixin(p, {
                exportOptions: e,
                layoutOptions: l
            });
            this.allLayerslegend && h.mixin(p.layoutOptions, {
                legendOptions: {
                    operationalLayers: this.allLayerslegend
                }
            });
            k = {
                Web_Map_as_JSON: s.toJson(t.fixJson(p)),
                Format: k.format,
            ...
            r._pendingDfd = this.async ? this.printGp.submitJob(k, b, null, e) : this.printGp.execute(k,
                b, e);
            return r
        },
        onComplete: function() {},
        ...
        _convertSvgSymbol: function(b) {
            if (!(8 >= z("ie")) && b.path) {
                this._canvasHolder || (this._canvasHolder = H.create("div"), this._canSurface = w.createSurface(this._canvasHolder, 200, 200));
        ...
        _convertSvgRenderer: function(b) {
            "simple" === b.type && b.symbol && b.symbol.path ? b.symbol = this._convertSvgSymbol(b.symbol) : "uniqueValue" === b.type ? (b.defaultSymbol && b.defaultSymbol.path && (b.defaultSymbol = this._convertSvgSymbol(b.defaultSymbol)), b.uniqueValueInfos && n.forEach(b.uniqueValueInfos, function(b) {
                    b.symbol.path && (b.symbol = this._convertSvgSymbol(b.symbol))
                },
                this)) : "classBreaks" === b.type && (b.defaultSymbol && b.defaultSymbol.path && (b.defaultSymbol = this._convertSvgSymbol(b.defaultSymbol)), b.classBreakInfos && n.forEach(b.classBreakInfos, function(b) {
                b.symbol.path && (b.symbol = this._convertSvgSymbol(b.symbol))
            }, this))
        },
        _createFeatureCollection: function(b, f) {
            var a = this._createPolygonLayer(),
                d = this._createPolylineLayer(),
                c = this._createPointLayer(),
            ...
            "esri.layers.FeatureLayer" === b.declaredClass && (a.layerDefinition.name = d.layerDefinition.name = c.layerDefinition.name = k.layerDefinition.name = b.name || b.id);
            b.renderer && !h.isFunction(b.renderer.attributeField) ? (a.layerDefinition.drawingInfo.renderer = b.renderer.toJson(), d.layerDefinition.drawingInfo.renderer = b.renderer.toJson(), c.layerDefinition.drawingInfo.renderer = b.renderer.toJson(), k.layerDefinition.drawingInfo.renderer = b.renderer.toJson()) : (delete a.layerDefinition.drawingInfo, delete d.layerDefinition.drawingInfo,
                delete c.layerDefinition.drawingInfo, delete k.layerDefinition.drawingInfo);
            var e = b.fields;
            ...
            e && (a.layerDefinition.fields = e, d.layerDefinition.fields = e, c.layerDefinition.fields = e, k.layerDefinition.fields = e);
            var l;
            for (l = 0; l < b.graphics.length; l++) {
                var m = b.graphics[l];
                if (!1 !== m.visible && m.geometry && (e = m.toJson(), !e.symbol || !(e.symbol.outline && "esriCLS" === e.symbol.outline.type))) {
                    e.symbol && (e.symbol.outline &&
                        e.symbol.outline.color && e.symbol.outline.color[3]) && (e.symbol.outline.color[3] = 255);
                    if (b.renderer && !e.symbol && (h.isFunction(b.renderer.attributeField) || b.renderer.proportionalSymbolInfo || "esri.renderer.DotDensityRenderer" === b.renderer.declaredClass || f))
                        if (f = f || b.renderer, e.symbol = f.getSymbol(m) ? f.getSymbol(m).toJson() : e.symbol, f.proportionalSymbolInfo)
                            if (f.getSize(m)) "point" === m.geometry.type || "multipoint" === m.geometry.type || "polygon" === m.geometry.type ? e.symbol.size = v.px2pt(f.getSize(m)) : "polyline" ===
                                m.geometry.type && (e.symbol.width = v.px2pt(f.getSize(m)));
                            else continue;
                    e.symbol && (e.symbol.path ? e.symbol = this._convertSvgSymbol(e.symbol) : e.symbol.text && delete e.attributes);
                    switch (m.geometry.type) {
                        case "polygon":
                            a.featureSet.features.push(e);
                            break;
                        case "polyline":
                            d.featureSet.features.push(e);
                            break;
                        case "point":
                            e.symbol && e.symbol.text ? g.featureSet.features.push(e) : c.featureSet.features.push(e);
                            break;
                        case "multipoint":
                            k.featureSet.features.push(e);
                            break;
                        case "extent":
                            e.geometry = C.fromExtent(m.geometry).toJson(),
                                a.featureSet.features.push(e)
                    }
                }
            }
            e = [];
            0 < a.featureSet.features.length && e.push(a);
            0 < d.featureSet.features.length && e.push(d);
            0 < k.featureSet.features.length && e.push(k);
            0 < c.featureSet.features.length && e.push(c);
            0 < g.featureSet.features.length && e.push(g);
        ...
        _getPrintDefinition: function(b) {
            ...
            this._preserveScale && h.mixin(a.mapOptions, {
                scale: this._outScale || E.getScale(b)
            });
        ...
        _createOperationalLayers: function(b) {
            ...
            for (f = 0; f < b.layerIds.length; f++)
                if (a = b.getLayer(b.layerIds[f]), a.loaded && a.visible && -1 === n.indexOf(k, a.id)) switch (d = a.declaredClass, c = {
                    id: a.id,
                    title: a.id,
                    opacity: a.opacity,
                    ...
                    case "esri.layers.ArcGISDynamicMapServiceLayer":
                        var e = [];
                        d = !!a._params.layers;
                        if (a._params.dynamicLayers) d = s.fromJson(a._params.dynamicLayers),
                            ...
                            if (d || a.layerDefinitions || a.layerTimeOptions) {
                                var l = a.createDynamicLayerInfosFromLayerInfos(),
                                    m = null;
                                d && (m = a.visibleLayers);
                                var m = u._getVisibleLayers(l, m),
                                    p = u._getLayersForScale(this._outScale || b.getScale(), l);
                                n.forEach(l, function(b) {
                                    if (!b.subLayerIds) {
                                        var c = b.id; - 1 < n.indexOf(m, c) && -1 < n.indexOf(p, c) && (b = {
                                                source: b.source.toJson()
                                            },
                                            a.layerDefinitions && a.layerDefinitions[c] && (b.definitionExpression = a.layerDefinitions[c]), a.layerTimeOptions && a.layerTimeOptions[c] && (b.layerTimeOptions = a.layerTimeOptions[c].toJson()), e.push({
                                                id: c,
                                                layerDefinition: b
                                            }))
                                    }
                                });
                                0 === e.length && (c.visibleLayers = [-1])
                            }
                        } else n.forEach(a.layerInfos, function(b) {
                            var c = {
                                id: b.id,
                                layerDefinition: {}
                            };
                            a.layerDefinitions && a.layerDefinitions[b.id] && (c.layerDefinition.definitionExpression = a.layerDefinitions[b.id]);
                            a.layerTimeOptions && a.layerTimeOptions[b.id] && (c.layerDefinition.layerTimeOptions =
                                a.layerTimeOptions[b.id].toJson());
                            (c.layerDefinition.definitionExpression || c.layerDefinition.layerTimeOptions) && e.push(c)
                ...
                for (f = 0; f < b.graphicsLayerIds.length; f++)
                    if (a = b.getLayer(b.graphicsLayerIds[f]), a.loaded &&
                        a.visible && -1 === n.indexOf(k, a.id)) switch (d = a.declaredClass, d) {
                        case "esri.layers.FeatureLayer":
                        case "esri.layers.LabelLayer":
                        case "esri.layers.CSVLayer":
                            d = null;
                            a.url && a.renderer && ("esri.renderer.ScaleDependentRenderer" === a.renderer.declaredClass ? "scale" === a.renderer.rangeType ? d = a.renderer.getRendererInfoByScale(b.getScale()) && a.renderer.getRendererInfoByScale(b.getScale()).renderer : "zoom" === a.renderer.rangeType && (d = a.renderer.getRendererInfoByZoom(b.getZoom()) && a.renderer.getRendererInfoByZoom(b.getZoom()).renderer) :
                                d = a.renderer);
                            if (d && ("esri.renderer.SimpleRenderer" === d.declaredClass || "esri.renderer.TemporalRenderer" === d.declaredClass || h.isString(d.attributeField) && a._getField(d.attributeField, !0)) && !d.proportionalSymbolInfo && "esri.renderer.DotDensityRenderer" !== d.declaredClass && "esri.layers.CSVLayer" !== a.declaredClass)
                                if (c = {
                                        id: a.id,
                                        title: a.id,
                                        opacity: a.opacity,
                                    ...
                                    }, c = h.mixin(c, this._getUrlAndToken(a)), "esri.renderer.TemporalRenderer" ===
                                    d.declaredClass && (l = c.layerDefinition.drawingInfo, l.latestObservationRenderer = l.renderer.latestObservationRenderer, l.trackLinesRenderer = l.renderer.trackRenderer, l.observationAger = l.renderer.observationAger, l.renderer = l.renderer.observationRenderer, a._trackIdField && (c.layerDefinition.timeInfo = {
                                    ...
                                    if (a._params.source &&
                                        (d = a._params.source.toJson(), h.mixin(c.layerDefinition, {
                                            source: d
                                        })), a.getDefinitionExpression() && h.mixin(c.layerDefinition, {
                                            definitionExpression: a.getDefinitionExpression()
                                        }), 2 !== a.mode) 0 < a.getSelectedFeatures().length && (d = n.map(a.getSelectedFeatures(), function(b) {
                                        ...
                                        d = n.map(a.getSelectedFeatures(), function(b) {
                                            return b.attributes[a.objectIdField]
                                        });
                                        ...
                                        d = null;
                                        a.getSelectionSymbol() && (d = new D(a.getSelectionSymbol()));
                                        h.mixin(c.layerDefinition.drawingInfo, {
                                            renderer: d && d.toJson()
                                        })
                                    } else c = this._createFeatureCollection(a);
                            else c = d && (d.proportionalSymbolInfo || "esri.renderer.DotDensityRenderer" === d.declaredClass) ? this._createFeatureCollection(a, d) : this._createFeatureCollection(a);
                            g.push(c);
                            this.allLayerslegend && this.allLayerslegend.push({
                                id: a.id
                            });
                        ...
                        case "esri.layers.GraphicsLayer":
                            c =
                                this._createFeatureCollection(a), g.push(c), this.allLayerslegend && this.allLayerslegend.push({
                                    id: a.id
                                })
                    }
                    b.graphics && 0 < b.graphics.graphics.length && (c = this._createFeatureCollection(b.graphics), g.push(c));
