...
define(["dojo/_base/array", "dojo/_base/lang", "dojo/has", "dojo/number", "dojo/_base/Color", "../kernel", "../lang", "../renderers/ClassBreaksRenderer", "../renderers/UniqueValueRenderer", "../symbols/SimpleMarkerSymbol", "../symbols/SimpleLineSymbol", "../symbols/SimpleFillSymbol"], function(t, v, O, u, q, P, E, x, F, y, z, A) {
    ...
    function B(a, b, k) {
        ...
        k = function(a, b, c) {
            ...
            if (2 == c) e = [a, b];
                ...
                for (d = 0; d < c; d++) f = 1 / (c - 1) * d, g = new q([Math.round(a.r +
                    f * (b.r - a.r)), Math.round(a.g + f * (b.g - a.g)), Math.round(a.b + f * (b.b - a.b))]), e.push(g);
            return e
        };
        if (1 == b) a = [a[0]];
        else if (2 == a.length)
            ...
            if (2 == b) a = [c, e];
                ...
                k = k(new q(l), new q(e), Math.ceil(p / 2));
                k.splice(0, 1);
                a = a.concat(k);
                if (p != b) {
                    b = [];
    ...
    function s(a, b, k, d) {
        ...
        0 >= g && console.log("Exception in Natural Breaks calculation");
        f /= g;
        g = 0;
    ...
    function C(a, b, k, d) {
        ...
        for (var m = c, r = !0; r || c < m;) {
            ...
            g = [];
            l = [];
            for (h = 0; h < d; h++) g.push(e[h]), l.push(p[h]), n = s(h, a, b, k), e[h] = n.sbMean, p[h] = n.sbSdcm, c += p[h]
        }
        if (c > m)
            for (h =
                0; h < d; h++) a[h] = f[h], e[h] = g[h], p[h] = l[h];
        return {
            mean: e,
            sdcm: p
        }
    ...
    function K(a, b, k) {
        ...
        for (r = 0; 2 > r && n; r++) {
            ...
            for (m = 0; m < l - 1; m++)
                for (; g[m + 1] + 1 != g[m + 2];)
                    if (g[m + 1] += 1, e = s(m, g, a, b), p = e.sbMean, c = e.sbSdcm, e = s(m + 1, g, a, b), h = e.sbMean, e = e.sbSdcm, c + e < f[m] + f[m + 1]) f[m] = c, f[m + 1] = e, d[m] = p, d[m + 1] = h, n = !0;
                    else {
                        g[m + 1] -= 1;
                        break
                    }
    ...
    function L(a, b, k, d, f, g, l) {
        ...
        for (e = 0; e < a.length; e++)
            if (p = a[e], h = p.attributes[b], k && (!d || "field" === d) ? h /= p.attributes[k] :
                "log" === d ? h = Math.log(h) / Math.log(10) : "percent-of-total" === d && l && (h *= 100, h /= l), E.isDefined(h)) {
        ...
        if ("equal-interval" == f.toLowerCase())
            ...
            if (0 < c.length)
                if (k = c[0], a = c[c.length - 1], c.length >= g && k !== a) {
                    ...
                    d = 0;
                    for (e = 1; e < g; e++) f = k + d - 1, f > c.length && (f = c.length - 1), 0 > f && (f = 0), n.push({
                        minValue: b,
                        maxValue: c[f]
