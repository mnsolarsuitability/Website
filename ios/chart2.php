...
var solarMax = <?php echo isset($_REQUEST['max']) ? $_REQUEST['max'] : "Math.max.apply(Math, solData.insolValue)"; ?>;
...
function draw(params) {
	...
	var x = d3.scale.ordinal()
		.domain(params.months.map(function(d) {
			return d.substring(0, 3);}))
		.rangeRoundBands([0, width/2], 0);

	var y = d3.scale.linear()
	    // SET Y AXIS HEIGHT
		.domain([0, (params.max + params.max*.1)])
		.range([height, 0]);
