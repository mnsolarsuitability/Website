	...
	webMercatorUtils, GeometryService) {
            ...
            BEQueryTask.execute(BEquery, function (results) {
                ...
                if (results.features && results.features.length > 0) {
                    ...
                    queryTask.execute(query, function (results) {
						...
						var insolation = '<div class="resultsTopicTitle">INSOLATION (kWh/m2)</div><div class="resultsDisplay">Total per Year: ' + y.toFixed(2) + warning + '<br>Avg per Day: ' + v.toFixed(2) + ' (' + quality + ')' + warning + warningMsg +'</div>';
                        ...
                        queryTask.execute(query, function (results) {
                            ...
                            var uShort ='<div class="resultsTopicTitle">Utility Service Provider:</div><div class="resultsDisplay">' + fullname + ' ' + phone + '</div>';

							//document.getElementById('results').innerHTML = document.getElementById('results').innerHTML + resultU;
							$("#results").html(insolation + uShort);

                            var uLong = "<p><b>Utility Service Provider:   </b><br />" + fullname + "<br />" + street + "<br />" + city + ", MN " + zip + "<br />" + phone + "<br /><a target='_blank' href='http://" + website + "'>" + website + "</a></p><p><a href='http://www.dsireusa.org/solar/incentives/index.cfm?re=1&ee=1&spv=1&st=0&srp=0&state=MN' target='_blank'>MN Incentives/Policies for Solar</a></p>" + getstarted + "<p>Report bad data <a href='/bad_data_handler.php?x=" + mp.x + "&y=" + mp.y + "' target='_blank'>here</a>.</p>";
							$("#resultsBigSol").html(insolation + uLong);
							
							//document.getElementById('resultsBigSol').innerHTML = result + resultBig; //"<iframe src='http://solar.maps.umn.edu/report.php?long=" + e.mapPoint.y + "&lat=" + e.mapPoint.x + "&y="+ y.toFixed(2) + "&u=" + utility + "' width='100%' height='100%'><p>Your browser does not support iframes.</p></iframe>";
                        });
                    });
		...
		function displayResults(results, messages) {
			...
			function draw(data, dataAttr, max, div, title) {
				...
				// create Solar Insol histo
				draw(data,data.insolValue,maxInsol, "#resultsHistoBig", "Solar Insolation By Month (kWh)");
				
				// create Sun Hrs histo
			    draw(data,data.sunHrValue,maxSun, "#sunHrHistoBig", "Sun Hours By Month");
